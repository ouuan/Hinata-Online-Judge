...
class SyncProblemDataHandler
{
	...
	private function compile_at_prepare($name, $config = array())
	{
		...
		if (isset($config['need_include_header']) && $config['need_include_header']) {
			exec("$cmd_prefix --add-readable-raw=$include_path/ /usr/bin/g++ -o $name {$config['src']} -I$include_path -lm -O2 -std=c++11 -DONLINE_JUDGE");
		} else {
			exec("$cmd_prefix /usr/bin/g++ -o $name {$config['src']} -lm -O2 -std=c++11 -DONLINE_JUDGE");
		}
