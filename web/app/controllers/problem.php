...
function handleUpload($zip_file_name, $content, $tot_size)
{
	...
	if ($is_in_contest) {
		...
		DB::query("insert into submissions (problem_id, submit_time, submitter, content, language, tot_size, status, result, is_hidden) values (${problem['id']}, now(), '{$submit_user}', '$esc_content', '$esc_language', $tot_size, '${result['status']}', '$result_json', {$problem['is_hidden']})");
	}
}
...
<div class="row d-flex justify-content-center">
	...
	<span class="badge badge-secondary mr-1"><?= UOJLocale::get('problems::memory limit') ?>: <?= $memory_limit != null ? "$memory_limit MiB" : "N/A" ?></span>
</div>
...
<ul class="nav nav-tabs" role="tablist">
	...
	<li class="nav-item"><a class="nav-link" href="/download.php?type=problem&id=<?= $problem['id'] ?>" role="tab"><span class="glyphicon glyphicon-download"></span> <?= UOJLocale::get('problems::download') ?></a></li>
